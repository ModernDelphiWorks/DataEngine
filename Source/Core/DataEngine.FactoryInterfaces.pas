...
unit DataEngine.FactoryInterfaces;
...
type
  TMonitorEventType = (teQueryStart, teQueryEnd, teFetchEnd, teError, teMetric);
  ...
  TMonitorParam = record
    ...
    RowsAffected: UInt32;
    Exception: Exception;
    constructor Create(const ACommand: String; AParams: TParams); overload;
    constructor Create(const AEventType: TMonitorEventType; const ACommand: String; AParams: TParams;
      const AExecutionTime: Int64 = 0; const AFetchTime: Int64 = 0; const ARowsAffected: UInt32 = 0;
      const AException: Exception = nil); overload;
  end;

  TMonitorProc = reference to procedure(const ACommand: TMonitorParam);
  TResilienceEvent = reference to procedure(const AAttempt: Integer; const AException: Exception; var ACancel: Boolean);

  IDBObserver = interface
    ['{89B25F68-91E0-469A-B8ED-0FE6AF4A2030}']
  ...
  TDBIsolationLevel = (ilDefault, ilReadUncommitted, ilReadCommitted, ilRepeatableRead, ilSerializable, ilSnapshot);

  TFetchMode = (fmAll, fmManual, fmOnDemand);

  TFetchOptions = record
    Mode: TFetchMode;
    BatchSize: Integer;
    constructor Create(const AMode: TFetchMode; const ABatchSize: Integer = 50);
  end;
  ...
  IDBDataSet = interface
    ...
    //
    property FetchOptions: TFetchOptions read _GetFetchOptions write _SetFetchOptions;
  ...
  IDBQuery = interface
    ...
    procedure _SetCommandText(const ACommandText: String);
    function _GetCommandText: String;
    function _GetParams: TParams;
    procedure ExecuteDirect;
    function ExecuteQuery: IDBDataSet;
    function RowsAffected: UInt32;
    procedure Prepare;
    procedure Unprepare;
    function ParamByName(const AValue: string): TParam;
    procedure _RefreshMetadata;
    function _GetFetchOptions: TFetchOptions;
    procedure _SetFetchOptions(const Value: TFetchOptions);
    function _GetSlowQueryThreshold: Integer;
    procedure _SetSlowQueryThreshold(const Value: Integer);
    property CommandText: String read _GetCommandText write _SetCommandText;
    property Params: TParams read _GetParams;
    property FetchOptions: TFetchOptions read _GetFetchOptions write _SetFetchOptions;
    property SlowQueryThreshold: Integer read _GetSlowQueryThreshold write _SetSlowQueryThreshold;
  end;

  IDBBulkLoader = interface
    ['{69A440D5-927A-4E38-9B41-B88DCE98862B}']
    function _GetTableName: string;
    procedure _SetTableName(const AValue: string);
    function _GetBatchSize: Integer;
    procedure _SetBatchSize(const AValue: Integer);
    procedure Prepare;
    procedure Execute(const ARows: Integer);
    procedure Clear;
    procedure SetValue(const AParamName: string; const ARidx: Integer; const AValue: Variant);
    function ParamByName(const AValue: string): TParam;
  ...
  IDBDataSetSnapshot = interface(IDBDataSet)
    ['{0B195E05-4B83-4EA2-97CC-63B0E8D76EFC}']
    function CreateView: IDBDataSet;
  end;

  IDBCacheProvider = interface
    ['{88B25F68-91E0-469A-B8ED-0FE6AF4A2028}']
  ...
  IDBMetadataCache = interface
    ['{8E1A3F5C-FD01-4B82-B2A4-D9E71F36E8B1}']
  ...
  IDBResiliencePolicy = interface
    ['{89B25F68-91E0-469A-B8ED-0FE6AF4A2029}']
    ...
    property OnRetry: TResilienceEvent read GetOnRetry write SetOnRetry;
  end;
...
begin
  EventType := teMetric; // Default to teMetric for backward compatibility if just command is passed
  Command := ACommand;
  Params := AParams;
  ExecutionTime := 0;
  FetchTime := 0;
...
constructor TFetchOptions.Create(const AMode: TFetchMode; const ABatchSize: Integer);
begin
  Mode := AMode;
