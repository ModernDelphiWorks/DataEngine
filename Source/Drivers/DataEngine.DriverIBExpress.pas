...
procedure TDriverIBExpress._InternalExecuteDirect(const ASQL: String; const AParams: TParams);
var
  LExeSQL: TIBQuery;
  LFor: UInt16;
begin
...
function TDriverQueryIBExpress._InternalExecuteQuery: IDBDataSet;
var
  LResultSet: TIBQuery;
  LFor: UInt16;
begin
  ...
  try
    ...
    try
      ...
      if LResultSet.RecordCount = 0 then
         (Result as TDriverDataSetIBExpress).FetchingAll := True;
    except
      on E: Exception do
      begin
    ...
    if (not Assigned(Result)) and Assigned(LResultSet) then
      LResultSet.Free;
  end;
end;
...
procedure TDriverQueryIBExpress._InternalExecuteDirect;
begin
  if _GetTransactionActive = nil then
    raise Exception.Create('Transaction not assigned.');
...
function TDriverQueryIBExpress._GetParams: TParams;
var
  LFor: Int16;
begin
...
procedure TDriverQueryIBExpress.Prepare;
begin
  FSQLQuery.Prepare;
end;

procedure TDriverQueryIBExpress.Unprepare;
begin
  FSQLQuery.Unprepare;
end;

function TDriverQueryIBExpress.ParamByName(const AValue: string): TParam;
begin
