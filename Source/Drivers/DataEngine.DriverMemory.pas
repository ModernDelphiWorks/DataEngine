...
begin
  ...
  try
    LCurrent := '';
    LInQuote := False;
    for LIndex := 1 to Length(AValueString) do
    begin
      LChar := AValueString[LIndex];
      if LChar = '''' then
...
procedure TDriverMemory._InternalExecuteDirect(const ASQL: String; const AParams: TParams);
begin
...
procedure TDriverQueryMemory._InternalExecuteDirect;
begin
...
function TDriverQueryMemory._InternalExecuteQuery: IDBDataSet;
begin
  if _GetTransactionActive = nil then
    raise Exception.Create('Transaction not assigned.');
...
function TDriverQueryMemory._GetParams: TParams;
begin
  Result := FDataSet.Params;
end;

procedure TDriverQueryMemory.Prepare;
begin
  // No-op for Memory driver
end;

procedure TDriverQueryMemory.Unprepare;
begin
  // No-op for Memory driver
end;

function TDriverQueryMemory.ParamByName(const AValue: string): TParam;
begin
  Result := FDataSet.Params.ParamByName(AValue);
end;
...
procedure TDriverDataSetMemory.ApplyUpdates;
begin
  // No-op for Memory driver
end;

procedure TDriverDataSetMemory.CancelUpdates;
begin
  // No-op for Memory driver
end;

function TDriverDataSetMemory.IsUniDirectional: Boolean;
begin
  Result := False;
end;

function TDriverDataSetMemory.IsReadOnly: Boolean;
begin
  Result := False;
end;

function TDriverDataSetMemory.IsCachedUpdates: Boolean;
begin
  Result := False;
end;

procedure TDriverDataSetMemory._SetUniDirectional(const Value: Boolean);
begin
  // No-op for Memory driver
end;

procedure TDriverDataSetMemory._SetReadOnly(const Value: Boolean);
begin
  // No-op for Memory driver
end;

procedure TDriverDataSetMemory._SetCachedUpdates(const Value: Boolean);
begin
  // No-op for Memory driver
end;
