...
procedure TDriverNexusDB._InternalExecuteDirect(const ASQL: String; const AParams: TParams);
var
  LExeSQL: TnxQuery;
  LFor: UInt16;
begin
  ...
  begin
    try
      ...
      FRowsAffected := 0; // NexusDB ExecQuery doesn't easily return rows affected here
    except
      on E: Exception do
      begin
...
function TDriverQueryNexusDB._InternalExecuteQuery: IDBDataSet;
var
  LResultSet: TnxQuery;
  LFor: UInt16;
begin
  ...
  try
    ...
    if LResultSet.RecordCount = 0 then
       (Result as TDriverDataSetNexusDB).FetchingAll := True;
  except
    on E: Exception do
    begin
...
procedure TDriverQueryNexusDB._InternalExecuteDirect;
begin
  if _GetTransactionActive = nil then
    raise Exception.Create('Transaction not assigned.');
...
function TDriverQueryNexusDB._GetParams: TParams;
var
  LFor: Int16;
begin
  ...
  begin
    Result.Add;
    Result[LFor].DataType := FnxQuery.Params[LFor].DataType;
    Result[LFor].Value := FnxQuery.Params[LFor].Value;
  end;
end;

procedure TDriverQueryNexusDB.Prepare;
begin
  FnxQuery.Prepare;
end;

procedure TDriverQueryNexusDB.Unprepare;
begin
  FnxQuery.Unprepare;
end;

function TDriverQueryNexusDB.ParamByName(const AValue: string): TParam;
begin
  Result := FnxQuery.ParamByName(AValue);
end;
