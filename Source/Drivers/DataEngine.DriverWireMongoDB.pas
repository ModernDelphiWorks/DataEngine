...
procedure TDriverWireMongoDB._InternalExecuteDirect(const ASQL: String; const AParams: TParams);
var
  LCommand: String;
begin
  try
    ...
    on E: Exception do
    begin
      _SetMonitorLog(ASQL, E.Message, AParams);
...
procedure TDriverQueryWireMongoDB._InternalExecuteDirect;
begin
  try
    if FDriverTransaction.TransactionActive = nil then
      raise Exception.Create('Transaction not assigned.');
...
function TDriverQueryWireMongoDB._InternalExecuteQuery: IDBDataSet;
var
  LUtil: IUtilSingleton;
...
begin
  ...
  try
    ...
    if LResultSet.RecordCount = 0 then
       (Result as TDriverResultSetWireMongoDB).FetchingAll := True;
  finally
